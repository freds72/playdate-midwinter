...
def PlotBinaryPatterns(Texture,nPatternRow,nPatternColumn):
    """This function creates a figure with a grid of thresholded versions of the 
      ...
      \note For the plots to show you have to invoke pyplot.show()."""
    # Figure=pyplot.figure();
    nPattern=nPatternRow*nPatternColumn;
    for i in range(16):
      # Figure.add_subplot(nPatternRow,nPatternColumn,i,xticks=[],yticks=[]);
      
      pattern = Texture # [i] # np.where(Texture*nPattern<i*np.size(Texture),1.0,0.0)
      print(f"Noise #{i}")
      print(pattern)
      G = np.zeros((8,8,1))
      ...
      img.save(f'./noise/noise{i}.png',bits=1,optimize=False)

      # pyplot.imshow(pattern, cmap="gray",interpolation="nearest");
...
def GenerateBlueNoiseDatabase(RandomSeedIndexList=range(1),MinResolution=16,MaxResolution=1024,ChannelCountList=[1,2,3,4],StandardDeviation=1.5):
    ...
    while(Resolution<=MaxResolution):
        OutputDirectory="./noise/%d_%d"%(Resolution,Resolution);
...
if(__name__=="__main__"):
    ...
    # GenerateNDBlueNoiseTexture((8,8,8,8),1,"/8x8/",1.9)
    # Texture=GetVoidAndClusterBlueNoise((8,8),1.9);
    ...
    for i in range(16):
      pattern = GetVoidAndClusterBlueNoise((8,8),1.9)
      print(f"Noise #{i}")
      G = np.zeros((8,8,1))
      # Where we set the RGB for each pixel
      t = (i/16)*64
