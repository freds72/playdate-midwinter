...
#define END_FUNC() do { \
	spall_buffer_end(&spall_ctx, &spall_buffer, pd->system->getElapsedTime()); \
} while(0)
#define BEGIN_BLOCK(name) do { \
	spall_buffer_begin(&spall_ctx, &spall_buffer, name, sizeof(name) - 1, pd->system->getElapsedTime()); \
} while(0)
#define END_BLOCK() do { \
	spall_buffer_end(&spall_ctx, &spall_buffer, pd->system->getElapsedTime()); \
} while(0)
#else
#define BEGIN_FUNC() do {} while(0)
#define END_FUNC() do {} while(0)
#define BEGIN_BLOCK(name) do {} while(0)
...
SPALL_FN SPALL_FORCEINLINE bool spall_buffer_begin_args(SpallProfile *ctx, SpallBuffer *wb, const char *name, signed long name_len, const char *args, signed long args_len, double when, uint32_t tid, uint32_t pid) {
    ...
    if (ctx->is_json) {
        char* buf = NULL;
        int buf_len = ctx->pd->system->formatString(&buf,
        ...
        int ret = spall__buffer_write(ctx, wb, buf, buf_len);
        ctx->pd->system->realloc(buf, 0);
        if (!ret) return false;
    } else {
